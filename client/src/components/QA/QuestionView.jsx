...
class QuestionView extends React.Component {
  ...
  markAnswerHelpful(event) {
    axios.put(`https://app-hrsei-api.herokuapp.com/api/fec2/hr-nyc/qa/answers/${event.target.id}/helpful`, { answer_id: event.target.id}, { headers: {Authorization: API_KEY}})
    .then((res) => console.log(res))
    .then(() => {
      this.getCurrentQuestions(this.props.productId);
    })
  ...
  reportAnswer(event) {
    ...
    axios.put(`https://app-hrsei-api.herokuapp.com/api/fec2/hr-nyc/qa/answers/${event.target.id}/report`, { answer_id: event.target.id}, { headers: {Authorization: API_KEY}})
    .then((res) => console.log(res))
    .catch((err) => { throw err; });
  }
